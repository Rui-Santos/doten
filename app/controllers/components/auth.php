 ...
 */
	function __setDefaults() {
		...
		$defaults = array(
			...
			'loginError' => __('呃..登录失败了.', true),
			'authError' => __('请先登录吧.', true)
		);
 ...
 */
	function login($data = null) {
		...
		if ($user = $this->identify($data, array('User.is_locked' => 0))) {
			/* write entire user data to session, sorry to use hard code */
			$userInstance = ClassRegistry::init('User');
			$user_data = $userInstance -> findById($user['id']);
